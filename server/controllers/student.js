...
const getStuListByCls = async function() {
  ...
  const pageSize = this.request.body.pageSize;
  const data = this.request.body;
  const result = await studentModel.getStuListCls(data.stu_major);
  let list = result.rows;
  let start = pageSize * (currentPage - 1);
  list = list.slice(start, start + pageSize);
  this.body = {
...
const addStudent = async function() {
  const data = this.request.body;
  let stu = await studentModel.addStudent(data.stu_id,data.stu_name,data.stu_gender,data.stu_age,data.stu_height,data.stu_weight,data.stu_tel,data.stu_major,data.stu_pay);
...
const updateInfo =async function(){
  const data = this.request.body;
  console.log(data)
  await studentModel.updateInfo(data.stu_id, data.stu_name, data.stu_gender, data.stu_age, data.stu_height,data.stu_weight, data.stu_tel, data.stu_major, data.stu_pay);
  this.body = {
    success: true,
    info: '会员信息更新成功！'
  }
