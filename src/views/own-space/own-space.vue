                ...
                >
                    ...
                    <FormItem label="手机：" prop="tel" >
                      <div style="display:inline-block;width:204px;">
                        <Input v-model="userForm.tel" ref="tel" @on-keydown="hasChangePhone"/>
                      </div>
                    </FormItem>
...
<script>
    ...
    methods: {
            ...
            this.$refs['userForm'].validate((valid) => {
                if (valid) {
                    ...
                    if(parseInt(Cookies.get('access'))===1){
                        this.url='http://localhost:9090/auth/coach/updateTel';
                    }
        ...
        init () {
            ...
            console.log("id:" + this.getUserInfo().id);
            let access = Cookies.get('access');
            console.log(access);
            if(access==1){
              this.$http.post('http://localhost:9090/auth/coach', this.form)
                .then((res) => {
                  if (res.data.success) {
                    console.log(res.data.data)
                    this.userForm.id=this.form.id;
                    this.userForm.name = res.data.data.coach_name;
                    this.userForm.tel = res.data.data.coach_phone;
                    // this.userForm.college = res.data.data.tea_college;
                    this.userForm.major = res.data.data.coach_strong;
                    this.initPhone = '';
                  } else {
                    console.log(res)
                    this.$Message.error(res.data.info);
